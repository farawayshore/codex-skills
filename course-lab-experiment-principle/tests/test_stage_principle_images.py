...
class StagePrincipleImagesTests(unittest.TestCase):
    ...
    def test_nested_output_dir_is_preserved_in_latex_image_path(self) -> None:
        with tempfile.TemporaryDirectory() as temp_name:
            root = Path(temp_name)
            decoded_dir = root / "pdf_decoded" / "sample"
            (decoded_dir / "images").mkdir(parents=True, exist_ok=True)
            ...
            out_dir = root / "results" / "sample" / "principle-images" / "lx1"
            out_tex = root / "results" / "sample" / "principle_figures.tex"
            out_json = root / "results" / "sample" / "principle_figures.json"
            ...
            entry = manifest["entries"][0]
            self.assertEqual(
                entry["latex_image_path"],
                f"principle-images/lx1/{entry['relative_output_path']}",
            )
            tex = out_tex.read_text(encoding="utf-8")
            self.assertIn(r"\includegraphics[width=0.92\columnwidth]{principle-images/lx1/", tex)
        ...
        with tempfile.TemporaryDirectory() as temp_name:
            ...
            payload = [
                ...
                {"type": "image", "img_path": "images/a.jpg", "image_caption": ["(a) 立体图"], "image_footnote": [], "page_idx": 0},
                {
                    ...
                    "image_caption": ["(b) 俯视图", "图 4-3-1 锥光干涉图示意图"],
                    "image_footnote": [],
                    "page_idx": 0,
                },
            ]
