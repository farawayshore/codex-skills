...
def build_manifest(decoded_json: Path, sections: dict[str, object], output_dir: Path) -> dict[str, object]:
    ...
    for idx, image in enumerate(images, start=1):
        ...
        try:
            path_parts = output_dir.relative_to(output_dir.anchor).parts if output_dir.anchor else output_dir.parts
        except ValueError:
            path_parts = output_dir.parts
        if "principle-images" in path_parts:
            root_index = path_parts.index("principle-images")
            latex_image_path = "/".join(path_parts[root_index:] + (destination_name,))
        else:
            latex_image_path = f"principle-images/{destination_name}"
        ...
        entries.append(
            {
                ...
                "latex_image_path": latex_image_path if exists else None,
...
def write_tex(manifest: dict[str, object], tex_path: Path) -> None:
    ...
    for block in blocks:
        ...
        if block_type == "subfigure_group":
            ...
            for position, entry in enumerate(entries):
                ...
                if entry.get("missing_source"):
                    ...
                    latex_path = latex_escape(
                        str(entry.get("latex_image_path") or f"principle-images/{entry['relative_output_path']}")
                    )
                    lines.append(rf"        \includegraphics[width=\linewidth]{{{latex_path}}}")
                lines.extend(
                    [
        ...
        if entry.get("missing_source"):
            ...
            continue
        rel_path = latex_escape(str(entry.get("latex_image_path") or f"principle-images/{entry['relative_output_path']}"))
        caption = latex_escape(str(entry["latex_caption"]))
        lines.extend(
            [
                r"\begin{figure}[H]",
                r"    \centering",
                rf"    \includegraphics[width=0.92\columnwidth]{{{rel_path}}}",
                rf"    \caption{{{caption}}}",
                rf"    \label{{{label}}}",
                r"\end{figure}",
            ]
