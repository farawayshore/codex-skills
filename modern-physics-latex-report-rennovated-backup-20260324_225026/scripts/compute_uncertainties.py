...
def markdown_report(input_path: Path, columns: list[tuple[str, dict[str, float | int]]], coverage_k: float) -> str:
    lines = [
        ...
        "",
        "| Quantity | n | Mean | Sample Std. Dev. | Type A | Resolution | Type B | Type C | Expanded |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for name, stats in columns:
        resolution = stats["resolution"]
        lines.append(
            "| {name} | {n} | {mean:.6g} | {sample_stddev:.6g} | {type_a:.6g} | {resolution} | {type_b:.6g} | {type_c:.6g} | {expanded_uncertainty:.6g} |".format(
                name=name,
                resolution="TBD" if resolution is None else f"{resolution:.6g}",
                **stats,
            )
        )
    lines.append("")
