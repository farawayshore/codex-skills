...
- Ask only for the missing or changed profile fields plus the instructor name and instructor title choice before drafting.
...
- When the user gives corrected metadata during a run, write those updates back into the profile so later experiments reuse them automatically.
