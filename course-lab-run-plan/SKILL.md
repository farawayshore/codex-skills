...
This standalone package uses only local copied tools under `/root/.codex/skills/course-lab-run-plan/`. It should reread normalized handout JSON and Markdown, route obvious handout cues into downstream leaf-skill buckets, and emit aligned JSON and Markdown artifacts that keep required work, enrichment opportunities, and unresolved gaps visible.
...
4. Review `run_readiness`, `leaf_skill_handoffs`, and `global_unresolved_gaps` in the JSON output.
