...
description: Use when building or refining a course lab report in this repo and the run needs course-aware discovery, handout or reference decoding, reusable workspace setup, picture-result analysis, PNG size control, appendix staging, and final QC without inventing missing content.
---

# Course Lab Report (rennovated)

Use this skill for course lab-report work in this repository.
...
```bash
python3 /root/.codex/skills/modern-physics-latex-report-rennovated/scripts/discover_sources.py \
  --course "Advanced Optics Laboratory" \
  --experiment "Interference Lab"
```

2. Create or refresh the reusable author profile:

```bash
python3 /root/.codex/skills/modern-physics-latex-report-rennovated/scripts/manage_author_profile.py
```

3. Create or reuse the workspace after the user confirms the course, experiment, mode, and template:

```bash
python3 /root/.codex/skills/modern-physics-latex-report-rennovated/scripts/ensure_report_workspace.py \
  --course "Advanced Optics Laboratory" \
  --experiment "Interference Lab" \
  --mode new \
  --template "/root/grassman_projects/AI_works/resources/latex_templet/Interference course.tex"
```

4. Normalize a decoded handout into report-ready section blocks:

```bash
python3 /root/.codex/skills/modern-physics-latex-report-rennovated/scripts/extract_decoded_sections.py \
  --decoded-json "/path/to/pdf_decoded/experiment/experiment.json" \
  --output-json /tmp/sections.json \
  --output-markdown /tmp/sections.md
```

5. Use `$course-lab-experiment-principle` after normalization and scaffold creation to write the Introduction, nearby Background, and Experiment Principle sections plus matching handout-derived theory figures directly into the report. Prefer normalized Markdown artifacts when they exist. For combined reports such as mechanics, run that leaf skill once per matching report part instead of trying to merge multiple handouts into one theory pass.

6. Stage experiment picture-result files when the handout requires recorded photos:

```bash
python3 /root/.codex/skills/modern-physics-latex-report-rennovated/scripts/stage_picture_results.py \
...
```bash
python3 /root/.codex/skills/modern-physics-latex-report-rennovated/scripts/stage_signatory_pages.py \
...
9. Hand data-processing work off to `$course-lab-data-processing` after the transferred data is confirmed:
...
10. Hand result reasoning off to `$course-lab-results-interpretation` only after normalized handout artifacts and processed-data artifacts are both stable, and use its artifact-only outputs before discussion ideas or final staging:
...
11. Hand discussion-idea generation off to `$course-lab-discussion-ideas` after stable interpretation artifacts and matched reference reports are available, and use its artifact-only outputs before discussion synthesis or final staging.
...
- Ask for the course name before discovery starts.
- Run `scripts/discover_sources.py --course "..." --experiment "..."`.
- Show the best-matching handout, reference-report, data, result-folder, and template candidates.
- Present all valid template candidates under `AI_works/resources/latex_templet/` outside `dont use`, and ask the user to choose one for the current run.
...
- Look for the handout under `AI_works/resources/experiment_handout/`.
- Prefer course-matching subtrees when the course name maps cleanly to local folders, but fall back to broader search under that root when it does not.
...
- Find matching reference reports under `AI_works/resources/lab_report_reference/`, prefer course-matching subtrees when possible, decode them when needed, and use them only to learn structure and depth, not to invent claims.
...
- Show all valid template candidates outside `dont use` and ask the user to choose one for the current run instead of silently defaulting to `tau_templet copy.tex`.
...
- Ask for the course name before asking repeated identity questions if it was not already confirmed during discovery.
...
- Treat `AI_works/resources/report_author_profile.json` as shared cross-course person metadata only. Do not persist the course name there by default.
...
- Ask only for missing or changed profile fields, the report language, the course-local instructor title or name, and any other course-local metadata that still need confirmation.
- Do not draft the title block until the course name, report language, and required metadata answers are present.
- After the user corrects metadata, write only the reusable person fields back into the author-profile file so later courses can reuse them.
...
- Use `$course-lab-experiment-principle` to handle early theory writing and handout-derived theory-image insertion before late evidence work starts.
- When a combined report has repeated parts such as `LX1` and `LX2`, run `$course-lab-experiment-principle` once per handout-part pair and pass the matching parent part so one run does not mutate sibling parts.
...
- Use `$course-lab-data-processing` after the data markdown is confirmed, and keep its processing artifacts separate from interpretation prose.
- Use `$course-lab-results-interpretation` after normalized handout and processed-data artifacts are ready so result inventory checks, handout-grounded interpretation, simulation/theory comparison, anomaly visibility, and unresolved comparison gaps are captured before discussion ideas or final staging begin.
...
- Principle writing and handout-derived theory-image insertion: `$course-lab-experiment-principle`
- Picture-result staging, evidence planning, signatory-page staging, and grouped-figure rules: `scripts/stage_picture_results.py`, `scripts/plan_picture_evidence.py`, `scripts/stage_signatory_pages.py`
...
- Do not skip `$course-lab-experiment-principle` when the handout uses decoded theory figures in the Introduction, nearby Background, or Experiment Principle sections.
- Do not leave handout-derived theory figures unmentioned in the nearby theory-facing body text.
