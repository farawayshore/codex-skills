...
1. Ask for the course name before discovery begins.
2. Run source discovery with both the course hint and the experiment query.
3. Show the best-matching handout, reference-report, data, result-folder, and template candidates.
4. Present all valid template candidates under `AI_works/resources/latex_templet/` outside `dont use`.
5. If discovery is weak, show the candidate list instead of pretending the match is certain.
6. Detect the existing result folder and existing `.tex` files before proposing modify, rewrite, or stop.
7. If multiple report `.tex` files exist and none is `main.tex`, ask which file is canonical before running the workspace script.
8. Confirm the experiment and ask whether to modify in place, rewrite from a fresh template, or stop before the workspace is touched.

## Phase 2: Handout And Source Readiness

9. Find the handout under `AI_works/resources/experiment_handout/`.
10. Prefer course-matching subtrees when they exist, but fall back to broader search under that root when they do not.
11. If the handout is missing, ask whether to provide one, continue without it, or stop.
12. Decode the handout with `$mineru-pdf-json` when no decoded marker exists.
13. Normalize the decoded handout with `scripts/extract_decoded_sections.py`.
14. Check from the handout whether the experiment requires picture-result analysis.
15. Check from the handout whether recorded result pictures or signatory pages will be required later.
16. Find reference reports under `AI_works/resources/lab_report_reference/`, preferring course-matching subtrees when possible, and decode them when needed.
17. Review the heading structure across the reference-report pool to learn the common report backbone before drafting.
18. If the handout needs picture-result analysis, find the corresponding files under `AI_works/resources/experiment_pic_results/`.
19. If required picture-result files are missing, ask whether to provide them, continue with placeholders, or stop.

## Phase 3: Workspace, Template, And Core Report Contract

20. Ensure `AI_works/results/<experiment-safe-name>/`.
21. Confirm the chosen template before copying it into the workspace.
22. Discover template candidates under `AI_works/resources/latex_templet/`.
23. Exclude templates under directories named `dont use` by default.
24. Ask the user to choose one of the valid templates for the current run instead of silently defaulting to `tau_templet copy.tex`.
25. Keep the procedures markdown file as `AI_works/results/<experiment-safe-name>/<experiment-safe-name>_procedures.md`.
26. Add a `\NeedsInput{...}` macro in the preamble if the template does not already define one.
27. Render every unanswered field and every `TBD` item in light red with that macro.
28. Mirror each procedure ID in TeX comments such as `% procedure:P01`.
29. Mirror each placeholder gap with its own nearby comment.

## Phase 4: Metadata And Front-Matter Setup

30. Create or load `AI_works/resources/report_author_profile.json`.
31. If the profile file is missing, create it automatically and then ask only for the missing values.
32. Treat that profile as shared cross-course person metadata instead of a place to persist course identity.
33. Ask for the report language before drafting the title block.
34. Ask only for missing or changed student, collaborator, affiliation, instructor, and other course-local front-matter fields.
35. Do not draft the title block until the course name, report language, and required metadata answers are present.
36. Write corrected reusable student and collaborator metadata back into the author profile for reuse across courses.
